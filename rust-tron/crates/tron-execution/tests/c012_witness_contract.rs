...
fn java_witness_oracle_replays_real_registry_commit_and_revoke(){
 ...
 for row in &all {
  ...
  let mut execution=manager.build_session().unwrap();let mut result=ActuatorResult::default();let actual=ActuatorRegistry::empty().execute(&full,&execution,Some(&mut result),ExecutionConfig{blackhole_address:blackhole.clone(),reward_callback:None,..ExecutionConfig::default()});
...
fn reward_callback_precedes_vote_account_reload_and_duplicate_order_is_preserved(){
 ...
 let(path,manager,mut outer)=session("callback");setup(&outer,"vote-duplicate-order");let contract=VoteWitnessContract{owner_address:OWNER.to_vec(),votes:vec![tron_protocol::protocol::vote_witness_contract::Vote{vote_address:CANDIDATE.to_vec(),vote_count:1};3],support:false};let any=Any{type_url:"type.googleapis.com/protocol.VoteWitnessContract".into(),value:contract.encode_to_vec()};let called=Arc::new(AtomicBool::new(false));let mut result=ActuatorResult::default();VoteWitnessActuator::new(any).unwrap().execute(&outer,Some(&mut result),ExecutionConfig{blackhole_address:vec![0x41;21],reward_callback:Some(Arc::new(Callback(called.clone()))),..ExecutionConfig::default()}).unwrap();assert!(called.load(Ordering::SeqCst));let stored=Account::decode(outer.store(StoreKind::Account).get(OWNER).unwrap().as_slice()).unwrap();assert_eq!(stored.balance,10);assert_eq!(stored.votes.len(),3);outer.revoke().unwrap();drop(manager);fs::remove_dir_all(path).unwrap();
}
