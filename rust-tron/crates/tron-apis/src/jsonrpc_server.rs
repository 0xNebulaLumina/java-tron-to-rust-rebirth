use std::{io, net::{IpAddr, Ipv4Addr, SocketAddr}, sync::Arc, time::Duration};
...
impl JsonRpcServerSet {
    pub fn new(config:JsonRpcServerConfig,context:ApiContext,method_config:TronJsonRpcConfig,filters:Arc<FilterManager>)->Result<Self,String>{
        ...
        for(surface,port)in config.enabled(){let backend=ContextJsonRpcBackend::new(context.clone(),surface.cursor(),filters.clone());let state=HandlerState{processor:Arc::new(JsonRpcProcessor::new(TronJsonRpcMethods::new(method_config.clone(),backend),config.limits.clone())),permits:Arc::new(Semaphore::new(config.max_concurrent_requests)),blocking:blocking.clone()};let router=Router::new().route("/",post(handle)).fallback(not_found).with_state(state);let mut http=HttpServerConfig::new(SocketAddr::new(IpAddr::V4(Ipv4Addr::UNSPECIFIED),port));http.request_deadline=config.request_deadline;http.first_request_timeout=config.first_request_timeout;http.connection_idle_timeout=config.connection_idle_timeout;http.connection_max_age=config.connection_max_age;http.controls.max_body_bytes=config.limits.max_request_bytes;http.controls.max_connections=config.max_connections;plans.push((surface,HttpServerPlan::new(http,router)));}
        Ok(Self{plans})
    }
    pub fn surfaces(&self)->impl Iterator<Item=JsonRpcSurface>+'_ {self.plans.iter().map(|v|v.0)}
    pub async fn serve(self,cancellation:watch::Receiver<bool>)->io::Result<()>{let mut tasks=tokio::task::JoinSet::new();for(_,plan)in self.plans{tasks.spawn(plan.serve(cancellation.clone()));}while let Some(result)=tasks.join_next().await{result.map_err(io::Error::other)??;}Ok(())}
}
