    ...
    pub async fn prepare(prepared: PreparedDeployment) -> Result<RuntimeAssembly, DeploymentError> {
        ...
        let dependencies=ProductionNodeDependencies {
            ...
            core_factory: Box::new(move |status,stop| {
                let runtime_config=ExecutionRuntimeConfig{actuator_registry:actuators.clone(),operation_registry,shielded_parameters:parameters.clone(),execution_config:ExecutionConfig{blackhole_address:blackhole,reward_callback:Some(Arc::new(ConsensusRewardCallback)),guard_representatives}};
