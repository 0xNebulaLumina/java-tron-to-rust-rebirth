...
Protocol implementations fail closed before schema recursion or comparison when a numeric ceiling is exceeded: 1,048,576 fixture-input bytes, 2,097,152 result-input bytes for each comparison operand, JSON depth 64, 20,000 JSON values, 262,144 UTF-8 bytes per string or object key, 2,097,152 emitted bytes, 1,000 schema errors, 1,000 reported mismatches, and 40,000 queued comparison nodes. Result inputs are incrementally read and capped, so comparison accepts every result the runner can emit through the declared emitted/result-input ceiling without buffering unbounded input. The checked-in `fixture-v1.schema.json` and `result-v1.schema.json` also bound collections and strings. A run-limit violation emits the schema-valid typed `fixture/INVALID_FIXTURE` result and exits 64; a comparison-limit violation emits a structured protocol error and exits 64. Implementations must not continue recursive validation, normalization, digesting, or comparison after a limit trips.

Governed platform subprocesses run one at a time, with at most 10 seconds per focused-cell child and 30 seconds for the top-level cell. Each stdout and stderr stream is incrementally drained and capped at 2,097,152 bytes; overflow terminates and reaps the child instead of accumulating further output. Fixture stdin is capped at 1,048,576 bytes, except the unit cell's deliberate one-byte-over-limit probe; comparison result files are independently capped at 2,097,152 bytes. The smoke cell permits at most 8 commands; the unit cell permits 64 fixture files and 16 child invocations; the differential cell permits 32 cases, 96 child invocations, 64 result files, and 134,217,728 total temporary result bytes; and the native-resource cell permits 40 child invocations, 35 result files, and 73,400,320 total temporary result bytes. All cells use private temporary directories and delete them before exit.
...
Every result records the Java revision pinned by `manifest.v1.json`, protocol version, implementation, and toolchain. Artifact names are `<case-id>.<runner>.result.v1.json` and `<case-id>.mismatch.v1.json`; content digests are SHA-256 over the exact emitted bytes.
