...
Regenerate the C000 case-level test/resource ledger at the pinned release revision `4a21592f95e37908b21bc3f611c6e7a1a67f09f3` and reject drift. Reconcile each stable case ID to its behavior claim, applicability decision, Rust fixture/test IDs, and passing regression command; many-to-one mappings require explicit decomposition. This is final zero-gap reconciliation, not first discovery. The C000 production and Java-test ownership ledgers remain generated inputs: C029 must preserve every stable ID and all reviewed ledger metadata and must not add C029 status or reconciliation fields to those ledgers.

The initial C029 implementation owns exactly three files: new read-only gate `tools/regression/c029_gate.py`, new versioned artifact `docs/oracles/c029-java-surface-reconciliation.v1.json`, and the existing registry `docs/oracles/manifest.v1.json`. The reconciliation JSON is initially authored and reviewed as a complete artifact containing a non-generic mapping for every source stable ID; it is not generated from the provisional C000 ledger and there is no separate seed or fourth planning artifact. `tools/reference-runner/generate-ledgers.py` is reused unchanged unless its read-only check exposes a separately reviewed C000 defect. Missing behavioral proof discovered during reconciliation is fixed only in the narrow owning C001-C028 fixture/test file, not pre-authorized as part of the initial file set and never by weakening the reconciliation.

The reconciliation artifact is an ordered exact join over every row in `docs/oracles/java-test-ownership.v1.json`. Its top level records schema name/version, chunk, pinned Java revision, source-ledger path/SHA-256/row count/ordered-identity SHA-256, gate source identity, exact accounting, deduplicated `proof_commands`, `compatibility_defects`, and ordered `rows`, with zero `unmapped` and `generic` counts. Each proof command has a stable ID, repository-relative cwd, exact argv array, bounded timeout, closed deterministic environment, result contract, and owning C029 item. Each row records the exact stable ID and source identity; one or more atomic behavior claims; typed applicability; owning item; concrete fixture, Rust test/scenario, proof-command, and observable-result links; explicit many-to-one behavior slices; semantic re-home domain and owning crate for framework-hosted protocol/crypto/chainbase/consensus behavior; required treatment for ignored, bounded-dynamic, assumption-gated, benchmark, and resource rows; determinism/isolation contract; and defect references. Every compatibility defect found through C028 records its existing tracker finding/source, affected stable IDs, minimal durable fixture, exact regression symbol/scenario, and proof command; non-compatibility review findings receive an explicit source-bound classification and rationale rather than a fabricated fixture.

`tools/regression/c029_gate.py --write` is an exact-ID merge/refresh operation during implementation only, never a bootstrap generator: it requires the existing complete, reviewed reconciliation artifact, fails without it, refreshes generated source envelopes, preserves reviewed payload for unchanged IDs, and fails on missing, duplicate, changed, orphaned, or newly unmapped stable IDs before one atomic replacement. Every newly discovered source stable ID must first receive a complete reviewed mapping in the authored artifact; `--write` must not synthesize, default, or accept an unreviewed mapping. `--check` is the only tracker-invoked mode and is strictly read-only. Before any proof command or pinned-Java source materialization, it records a baseline that includes pre-existing tracked and untracked repository state without modifying it. All proof execution and materialization occur in disposable temporary/worktree isolation. An all-exit `finally` path handles success, command failure, timeout, signal, internal exception, and early fail-fast return: it compares the original repository with the baseline, compares the disposable execution tree with its pristine seed, cleans only gate-created disposable resources, and fails on any delta. It must never pre-clean, overwrite, reset, or otherwise touch pre-existing user changes. Within that guard it authenticates the ledger and central manifest, validates the schema and exact ordered bijection, rejects generic or dangling mappings and undecomposed shared proofs, confirms concrete Rust symbols/scenarios, executes each deduplicated proof command exactly once, rejects skipped or zero-test success, checks ignored/re-home/defect accounting, applies fresh temporary directories, injected/virtual time, ephemeral listeners, fixed seeds and immutable inputs as declared, performs required repeatability comparisons without rerunning unaffected commands, and proves representative bypass mutations are rejected in memory.

C029.01 owns pinned-ledger regeneration and exact source/count/order/stable-ID comparison. C029.02 owns per-row atomic claims, applicability, concrete proof/result links, and many-to-one decomposition. C029.03 owns semantic re-homing of framework-hosted protocol/crypto/chainbase/consensus behavior. C029.04 owns explicit ignored, shielded, VM, benchmark, bounded-dynamic, assumption-gated, and resource treatment. C029.05 owns exhaustive classification of C001-C028 review findings and a minimal durable regression fixture for every actual compatibility defect. C029.06 owns command deduplication, deterministic environments, fresh isolation, repeatability projections, and removal of sleeps, fixed ports, ambient time/randomness, shared paths, external network dependencies, and mutable images from reachable proofs.

C029.06 repository-trusts the complete Linux `x86_64-unknown-linux-gnu` Rust `1.85.1` installation selected by `rust-tron/rust-toolchain.toml`: profile `minimal`; requested components `cargo`, `clippy`, `rust-docs`, and `rustfmt`; requested targets `aarch64-apple-darwin`, `aarch64-unknown-linux-gnu`, `x86_64-apple-darwin`, and `x86_64-unknown-linux-gnu`; installed component inventory `cargo-x86_64-unknown-linux-gnu`, `clippy-preview-x86_64-unknown-linux-gnu`, `rust-docs-x86_64-unknown-linux-gnu`, the four requested `rust-std-*` targets, `rustc-x86_64-unknown-linux-gnu`, and `rustfmt-preview-x86_64-unknown-linux-gnu`. The exact rustlib manifest inventory is `components`, the nine matching `manifest-*` files, `multirust-channel-manifest.toml`, `multirust-config.toml`, and `rust-installer-version`; the respective channel/config/components SHA-256 digests are `7e97a1235ea88353e5b930cd19308477d1fcb854e1c69bd4f1a5140e087e54d2`, `2ef02fd1c74593cb7bbd30682fd1019fab6e16c3a6b98b0ce453b323aa149f50`, and `b681eda7dd3a3d83dc7a2144788a974c62ab3b883e3cbee3a9191dc7aedf1ca7`. The canonical sorted path/type/size/file-SHA-256 inventory contains 46,889 entries (45,598 files, 1,291 directories, 1,583,329,647 file bytes) and has SHA-256 `1ffe6c8e2e9704b5c60978de8e1c1a555aa2e9f9e927b196bfb7a41d565ae3b0`; `bin/cargo` and `bin/rustc` have SHA-256 `87e9ff02f95ea17d877e7c4eeb0f365f50fe3827f5afec132131ff3b8031a57e` and `f8d033fd72e878ec60493788c8bffec2e6c154dcab6f0cb4c145af8ada26a7ed`. The gate authenticates every toolchain file against this inventory before use, materializes the entire installation read-only below its invocation-private `RUSTUP_HOME` without a shared toolchain symlink, preserves Cargo.lock checksum authentication in a private read-only registry, and rechecks shared toolchain/cache integrity from the outer cleanup path on success, ordinary failure, proof timeout, or handled signal before exit.

Before C029.01 enters `doing`, freeze this exact ordered, fail-fast, read-only `C029.V` command list: `(.) python3 tools/reference-runner/generate-ledgers.py --check`, then `(.) python3 tools/regression/c029_gate.py --check`. The first command proves immutable C000 ledger regeneration; the second performs the complete reconciliation, current-tree proof execution, defect, determinism, mutation, final accounting, and clean-tree checks while deduplicating proof commands internally. Do not store separate metadata/Rust/all invocations that would execute the same proofs twice.

Review is sliced in order: (1) ledger identity, stable-ID/accounting, merge safety, and manifest registration; (2) behavior claims, applicability, many-to-one decomposition, and semantic re-home by domain; (3) ignored/dynamic/assumption/resource and benchmark/blocker decisions; (4) concrete current-tree fixture/test dispatch and historical defect closure; (5) deterministic execution, isolation, bounded timeouts, immutable inputs, and no mutation; (6) final zero-gap accounting and exact stored-command review. A changed artifact schema, ledger identity, normalization/result contract, fixture mapping, or gate command resets C029.V and review.

**Gate:** regeneration is clean; the reconciliation is an exact ordered bijection over the pinned case ledger; no case or atomic claim is unexplained, generic, silently omitted, or without an applicable passing current-tree result; every actual compatibility defect has a minimal durable fixture; deterministic isolated execution and mutation guards pass; the gate leaves the repository unchanged.

### C030 — Multi-node endurance, security, supply-chain, and performance qualification

Tracker item order and stable item boundaries remain unchanged, but execution order within C030 is explicit: complete `C030.09` first, then run the governed `C030.01` through `C030.08` scenarios. The tracker validator has one narrow next-item exception: while C030 is current and C030.09 is unfinished, `--next` selects C030.09; after C030.09 is done, it resumes ordinary array order at the first unfinished C030.01-C030.08 item. All other chunks always retain ordinary array-order selection. Before execution, C030.09 freezes the release-gate specification with exact versions/config/topology/workloads, numeric duration/block/maintenance/fork minima, sampling frequency, zero/tolerance invariants, performance baselines/budgets, security severity and closure policy, license authorities, and clean-environment definition. This dependency does not renumber items or change their acceptance semantics.
